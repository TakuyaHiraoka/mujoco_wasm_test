...
function evaluateDiskThreading(segments, { center, normal, radius, gapOut, gapHalfAngleRad }, wireRadius) {
  const { u, v } = buildDiskFrame(normal, gapOut);
  const planeTol = wireRadius * 0.18 + 1e-7;
  const gapMarginRad = Math.asin(clamp((wireRadius * 1.15) / Math.max(radius, wireRadius * 1.4), 0, 0.72));
  ...
  for (const segment of segments) {
    ...
    if (Math.abs(da) <= planeTol && Math.abs(db) <= planeTol) continue;
    const straddlesPlane = (da <= planeTol && db >= -planeTol) || (da >= -planeTol && db <= planeTol);
    ...
    const penetration = clamp((deepInteriorRadius - radial) / Math.max(deepInteriorRadius, 1e-6), 0, 1);
    if (radial <= deepInteriorRadius) {
      if (throughGap) gapCrossings += 1;
      else crossings += 1;
...
export function runStaticDiagnostics(spec) {
  ...
  checks.push(makeCheck(
    ...
    threading.nongapCrossings >= 1 ? '少なくとも一方の主ループ面を相手片が貫いており、知恵の輪らしい噛み合いがある。' : threading.gapCrossings >= 1 ? '貫通は gap 近傍のみ。やや抜けやすい。' : '開始時の位相的噛み合いが弱い可能性。',
  ));
  checks.push(makeCheck(
    'クリア閾値 clearance 余裕',
    solveClearanceMargin > Math.max(spec.wire.radius * 1.2, 0.03) ? 'pass' : solveClearanceMargin > 0 ? 'warn' : 'fail',
    `${solveClearanceMargin.toFixed(4)} m`,
    solveClearanceMargin > Math.max(spec.wire.radius * 1.2, 0.03) ? '開始状態より十分大きい clearance が必要。' : solveClearanceMargin > 0 ? '余裕が小さい。早すぎるクリアに注意。' : '開始状態で既に閾値を満たしている。判定が緩すぎる。',
  ));
  checks.push(makeCheck(
    'IR メカニズム',
    'info',
...
export function buildRuntimeDiagnostics(spec, pose) {
  ...
  const clearanceTarget = Math.max(spec.geometry.solveClearance ?? Math.max(spec.wire.radius * 2.4, 0.03), startClearance + Math.max(spec.wire.radius * 1.2, 0.03));
  const clearanceProgress = clamp(
    (interPieceClearance - startClearance) / Math.max(clearanceTarget - startClearance, spec.wire.radius * 1.1, 1e-6),
    0,
    1,
  );
  ...
  const distanceProgress = clamp(
    (centerDistance - spec.geometry.startCenterDistance) / Math.max(spec.geometry.solveDistance - spec.geometry.startCenterDistance, spec.wire.radius * 4.0, 1e-6),
    0,
    1,
  );
  let separation = clamp(0.50 * threadingProgress + 0.25 * clearanceProgress + 0.25 * distanceProgress, 0, 1);
  if (threading.nongapCrossings > 0) separation = Math.min(separation, 0.68);
  else if (threading.gapCrossings > 0 || threading.score > 0.08) separation = Math.min(separation, 0.88);
  ...
  if (threading.nongapCrossings > 0) warnings.push('まだ主ループ面の貫通が残っています。未クリアです。');
  else if (threading.gapCrossings > 0) warnings.push('まだ gap 近傍に貫通が残っています。あと少し通し切る必要があります。');
  if (separation > 0.9 && (threading.nongapCrossings > 0 || threading.score > 0.08)) warnings.push('分離度が高すぎます。位相貫通が残っている可能性があります。');
  return {
    centerDistance,
    segmentDistance,
    interPieceClearance,
...
export function formatRuntimeDiagnostics(diag) {
  const lines = [
    ...
    `threadingCrossings=${diag.threadingCrossings}`,
    `threadingGapCrossings=${diag.threadingGapCrossings}`,
    `threadingScore=${diag.threadingScore.toFixed(3)}`,
